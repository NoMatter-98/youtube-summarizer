...
- [ ] **生成包含时间戳的摘要**
  - [ ] **设计 Prompt:** 调整向 Gemini **进行总结**的 Prompt。在输入中包含我们从阶段二获取到的带时间戳的字幕，并要求它在生成总结时，能在关键结论后附上对应的时间点（例如 `[02:15]`）。

- [ ] **前端脚本改造 (`background.js` 或 `content.js`)**
  - [ ] **解析时间戳:** 在注入前端的脚本中，增加一个函数，用于通过正则表达式查找并解析摘要文本中的时间戳（例如 `[HH:MM:SS]` 或 `[MM:SS]` 格式）。
  - [ ] **创建超链接:** 将所有解析出的时间戳文本，动态地渲染成可点击的 `<a>` 标签。
  - [ ] **添加点击事件:**
    - [ ] 为这些 `<a>` 标签绑定点击事件。
